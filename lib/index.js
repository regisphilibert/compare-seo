...
export default async function(pages = [], params = {} ) {
  const { metas = [], excludes = [], gauge = false, transformItems = false } = params
  ...
  if(pages.length) {
    for (const page of pages) {
      ...
      if(comparison.base.status == "ok" && comparison.against.status == "ok") {
        const baseSEO = getSEOMetas(comparison.base.doc, metas, excludes, transformItems)
        const againstSEO = getSEOMetas(comparison.against.doc, metas, excludes, transformItems)
        let logs = []
