...
export const getSEOMetas = (doc, meta_names_user = [], excludes = [], transformItems = false) => {
  ...
  let metas = [...doc_metas].filter(meta => typeof meta.getAttribute('content') != "undefined").map(meta => {
    const name = meta.getAttribute('name') ? meta.getAttribute('name') : meta.getAttribute('property')
    let value = meta.getAttribute('content') || "undefined"
  ...
  if(transformItems) {
    metas = metas.map(transformItems)
  }
